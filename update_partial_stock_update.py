...
def get_snapdeal_product(cur_products, product_id):
    product_query = "SELECT snapdealp_portal_sku, snapdealp_portal_unique_id, snapdealp_account_id from snapdealp_snapdealpproducts where product_id = " + str(
        product_id)
    cur_products.execute(product_query)
    product = cur_products.fetchone()
    if product is not None:
...
def lambda_handler(event, context):
    ...
    # snapdeal file
    snapdeal_file_name = 'snapdeal_partial_portal_update10_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
    snapdeal_file_from = '/tmp/' + snapdeal_file_name
    snapdeal_file_to = '/buymore2/bin_reco/master_stock/' + snapdeal_file_name
    snapdeal_portal_update_csv = open(snapdeal_file_from, 'w')
    snapdeal_fieldnames = ['product_id', 'quantity', 'message', 'status']
    ...
    # Fetch the stocks updated
    ##delhi
    print("delhi location stock update")
    update_stock_ids_str = ",".join([str(id) for id in update_stock_ids])
    stock_query = "select * from api_masterstock where masterstock_id in (" + update_stock_ids_str + ") and warehouse=10"
    cur_orders.execute(stock_query)
    stocks = cur_orders.fetchall()
    for stock in stocks:
        product_id = stock[2]
        ...
        # print("quantity=794=>",quantity)
        # print("snapdeal_quantity=795=>", snapdeal_quantity)
        # print("snapdeal_csv_row==796=>",snapdeal_csv_row)
    ...
    with open(snapdeal_file_from, 'rb') as f:
        data = dbx.files_upload(f.read(), snapdeal_file_to, mode=dropbox.files.WriteMode.overwrite)
    ###delhi snapdeal close
    # Todo: Add new portal file upload code here

    # close db connections
    # conn_products.close()
    # conn_orders.close()
    ##snapdeal bangalore code
    # snapdeal file
    print("snapdeal bangalore invenetory update")
    snapdeal_file_name = 'snapdeal_partial_portal_update4_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
    snapdeal_file_from = '/tmp/' + snapdeal_file_name
    snapdeal_file_to = '/buymore2/bin_reco/master_stock/' + snapdeal_file_name
    snapdeal_portal_update_csv = open(snapdeal_file_from, 'w')
    snapdeal_fieldnames = ['product_id', 'quantity', 'message', 'status']
    ...
    # # Fetch the stocks updated
    update_stock_ids_str = ",".join([str(id) for id in update_stock_ids])
    stock_query = "select * from api_masterstock where masterstock_id in (" + update_stock_ids_str + ") and warehouse=4"
    cur_orders.execute(stock_query)
    stocks = cur_orders.fetchall()
    for stock in stocks:
        product_id = stock[2]
    ...
    with open(snapdeal_file_from, 'rb') as f:
        data = dbx.files_upload(f.read(), snapdeal_file_to, mode=dropbox.files.WriteMode.overwrite)
    ###snapdeal bangalore code close
    ##kolakata snapdeal code starts
    # # snapdeal file
    snapdeal_file_name = 'snapdeal_partial_portal_update7_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
    snapdeal_file_from = '/tmp/' + snapdeal_file_name
    snapdeal_file_to = '/buymore2/bin_reco/master_stock/' + snapdeal_file_name
    snapdeal_portal_update_csv = open(snapdeal_file_from, 'w')
    snapdeal_fieldnames = ['product_id', 'quantity', 'message', 'status']
    ...
    # # Fetch the stocks updated
    update_stock_ids_str = ",".join([str(id) for id in update_stock_ids])
    stock_query = "select * from api_masterstock where masterstock_id in (" + update_stock_ids_str + ") and warehouse=7"
    cur_orders.execute(stock_query)
    stocks = cur_orders.fetchall()
    for stock in stocks:
        product_id = stock[2]
    ...
    ##kolakata code ends
    # # Todo: Add new portal file upload code here
    ###mumbai snapdeal code
    # # snapdeal file
    snapdeal_file_name = 'snapdeal_partial_portal_update3_' + str(int(datetime.timestamp(datetime.now()))) + '.csv'
    snapdeal_file_from = '/tmp/' + snapdeal_file_name
    snapdeal_file_to = '/buymore2/bin_reco/master_stock/' + snapdeal_file_name
    snapdeal_portal_update_csv = open(snapdeal_file_from, 'w')
    snapdeal_fieldnames = ['product_id', 'quantity', 'message', 'status']
    ...
    # Fetch the stocks updated
    update_stock_ids_str = ",".join([str(id) for id in update_stock_ids])
    stock_query = "select * from api_masterstock where masterstock_id in (" + update_stock_ids_str + ") and warehouse=3"
    cur_orders.execute(stock_query)
    stocks = cur_orders.fetchall()
    for stock in stocks:
        product_id = stock[2]
