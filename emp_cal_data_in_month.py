...
def lambda_handler(event, context):
    ...
    try:
        ...
        sql.to_csv('/tmp/a0.csv', index=False)
        sql0=pd.read_csv('/tmp/a0.csv')
        ...
        print("112")
        sql0['factor'] = float(sql0['ctc']/float(sql0['ctc']-float(sql0['pf_employer']+sql0['pf_employee']+sql0['pt']+sql0['esi_employer']+sql0['esi_employee'])))
        print(sql0['factor'][0])
        a=sql0['factor'][0]
        test1=a*float(lop)
        print("test1",test1)
        test2=float(no_of_days)-test1
        print("test2",test2)
        test3=test2/float(no_of_days)
        print("test3",test3)
        sql0.to_csv('/tmp/a1.csv', index=False)
        sql1 = pd.read_csv('/tmp/a1.csv')
        print("116")

        #CTC*((No Of Days - LOP)/No Of Days))
        sql1['ctc1'] = sql1['ctc'] * float((float(no_of_days)-float(lop))/float(no_of_days))
        print("120")
        ###Basic*((No Of Days - (Factor*LOP))/No Of Days
        sql1['basic1'] = sql1['basic'] * test3
        print("123")
        ###HRA*((No Of Days - (Factor*LOP))/No Of Days
        sql1['hra1'] = sql1['hra'] * test3
        ##Conveyance*((No Of Days - (Factor*LOP))/No Of Days
        sql1['conveyance_allowances1'] = sql1['conveyance_allowances'] * test3
        ##medical*((No Of Days - (Factor*LOP))/No Of Days
        sql1['medical_allowance1'] = sql1['medical_allowance'] * test3
        ###CCA*((No Of Days - (Factor*LOP))/No Of Days
        sql1['cca_allowance1'] = sql1['cca_allowance'] * test3
        ###Special*((No Of Days - (Factor*LOP))/No Of Days
        sql1['special_allowances1'] = sql1['special_allowances'] * test3
        print("133")
        ...
        sql1.to_csv('/tmp/a2.csv', index=False)
        sql5 = pd.read_csv('/tmp/a2.csv')
        sql5['ctc'] = sql1['ctc1']
        ...
        sql5.to_csv('/tmp/a7.csv', index=False)
        data = pd.read_csv('/tmp/a7.csv')
        ...
        data1.to_csv('/tmp/a9.csv', index=False)
        print("true")
        if data1.empty:
            print("no data found")
