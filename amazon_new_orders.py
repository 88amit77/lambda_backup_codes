...
def get_accounts():
    accounts = []
    conn_products = psycopg2.connect(database="products", user=name, password=password,
...
def lambda_handler(event, context):
    ...
    print(max_time)
    print(now_time)
    conn_orders = psycopg2.connect(database="orders", user=name, password=password,
                                   host=rds_host, port="5432")
    conn_products = psycopg2.connect(database="products", user=name, password=password,
                                     host=rds_host, port="5432")
    ...
    for account in accounts:
        ...
        if now_time >= min_time and now_time <= max_time:
            ...
            print('3 hours')
            service_status = orders_api.list_orders(marketplaceids=(account['MARKETPLACE_ID']),
                                                    created_after=datetime.now() - timedelta(hours=3))
            created_after = datetime.now() - timedelta(hours=3)
            print("created_after=====", created_after)
        ...
        ### added code for list_orders_by_next_token
        while True:
            try:
                next_token = str(orders_response['NextToken']['value'])
                service_status1 = orders_api.list_orders_by_next_token(next_token)
                orders_response1 = service_status1.parsed
                temp_order = orders_response1['Orders']['Order']
                if isinstance(temp_order, list):
                    orders += temp_order
                else:
                    orders += [temp_order]
                next_token = str(orders_response1['NextToken']['value'])
                # print(orders_response1)
                # orders+ = orders_response1['Orders']['Order']
                # token = orders_response1['NextToken']['value']
            except:
                break
        ### added code for list_orders_by_next_token close

        print("orders220====", orders)
        for order in orders:
            ###have to check order_id  present in new order or not if not present then only go ahead
            order_id = order['AmazonOrderId']['value']
            cur_orders.execute(
                "select * from api_neworder where order_id = '" + str(order_id) + "' and portal_id = 1 limit 1")
            exist_orders = cur_orders.fetchall()
            if len(exist_orders):
                continue
            ###code end
            try:
